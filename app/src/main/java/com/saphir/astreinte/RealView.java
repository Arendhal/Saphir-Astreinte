...
/**
 * RealView allows users to switch between multiple screens (layouts) in the same way as the Android home screen (Launcher application).
 * <p>
 * You can add and remove views using the normal methods {@link ViewGroup#addView(View)}, {@link ViewGroup#removeView(View)} etc. You may want to listen for updates by calling {@link RealView#setOnScreenSwitchListener(OnScreenSwitchListener)}
 ...
 * @version 0.1.0
 */
public class RealView extends ViewGroup {
	...
	/**
	 * Listener for the event that the RealView switches to a new view.
	 */
	...
	public RealView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (widthMode != MeasureSpec.EXACTLY) {
			throw new IllegalStateException("View switcher can only be used in EXACTLY mode.");
		}
