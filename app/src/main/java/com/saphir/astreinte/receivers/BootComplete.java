	...
	public void onReceive(Context context, Intent intent) {
        ...
        for(cursor.moveToFirst();!cursor.isAfterLast();cursor.moveToNext()) {
        	...
        	if(System.currentTimeMillis()<timer.stopTime) {
	    		...
	    		//Log.d("BootComplete","subCursorCount="+subCursor.getCount());
	            ...
	            for(int i=0; i<subCursor.getCount(); i++) {
	            	...
	            	//Log.d("BootComplete","title="+subCursor.getString(subCursor.getColumnIndex(Timers.TITLE))+", interval="+subCursor.getInt(subCursor.getColumnIndex(Timers.INTERVAL)));
	            	if(findNextIntervalIteration==0) {
	            		//Log.d("BootComplete","timer.id="+timer.id+", subCursorTimerId="+subCursor.getInt(subCursor.getColumnIndex(Timers.TIMER_ID)));
		            	...
		            	//Log.d("BootComplete","cumulativeTimeElapsed="+sdf.format(cumulativeTimeElapsed)+", title="+subCursor.getString(subCursor.getColumnIndex(Timers.TITLE))+", interval="+subCursor.getInt(subCursor.getColumnIndex(Timers.INTERVAL)));
	            ...
	            if(nextTimerId!=0) {
	            	...
	            	//Log.d("BootComplete","Next timer: Title="+nextTimer.title+",startTime="+sdf.format(nextTimer.startTime)+", stopTime="+sdf.format(nextTimer.stopTime));
	            }
